...
// User has to provide a instance of Config that can be written to file
// User can specify the location of the output file

use crate::config::{Config, self, NodeID, MacAdress};
use std::io::Write;
use std::fs;

pub fn generate(config: &Config, path: &String) {
    ...
    // writeln!(file, "static config: Config = Config::new(nodes, relay, black_list)").unwrap();
}

fn write_nodes(file: &mut fs::File, config: &Config) {
...
fn write_relay(file: &mut fs::File, config: &Config) {
    let relay = node_id_to_string(&config.relay());
    writeln!(file, "    relay: {}", relay).unwrap();
}

fn write_black_list(file: &mut fs::File, config: &Config) {
